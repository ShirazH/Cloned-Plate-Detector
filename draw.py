...
def draw(plates):
    found = plates
    #Set character start and end for getting frame indices from frame images
    p_start = 's\\'
    p_end = '.'
    ...
    for r in range(0,len(found)):
        ...
        for path in glob.iglob(r'C:\uni_project\final-year-project-vmmr-cmd\yolo_gpu_large\combining\frames\*.bmp'):
            ...
            if(str(found[r].frame) == str(frame_index)):
                #Update draw count for each frame, this is used for y positioning of text written on the frame
